...
public class MenuScriptMine : MonoBehaviour
{
    Transform menuPanel;
    Event keyEvent;
    ...
    void Start()
    {
 //GameManager.GM.accept = KeyCode.F;

        menuPanel = transform.Find("ControlPanel");
       
                //KEYBOARD
                PCPanel =  menuPanel.GetChild(1).gameObject;
                ...
                //GAMEPAD
                JoystickPanel = menuPanel.GetChild(5).gameObject;
                ...
                //otherPanels
                MainPanel =  menuPanel.GetChild(0).gameObject;
                SoundPanel = menuPanel.GetChild(2).gameObject;
                SettingPanel = menuPanel.GetChild(3).gameObject;
                ControlPanel = menuPanel.GetChild(4).gameObject;
                ...
                MainPanel.transform.GetChild(0).gameObject.GetComponent<TextMeshProUGUI>().text = ImportText.allPhrases["continue"][PlayerPrefs.GetString("language")]; 
                MainPanel.transform.GetChild(1).gameObject.GetComponent<TextMeshProUGUI>().text = ImportText.allPhrases["settings"][PlayerPrefs.GetString("language")]; 
                MainPanel.transform.GetChild(2).gameObject.GetComponent<TextMeshProUGUI>().text = ImportText.allPhrases["titleScreen"][PlayerPrefs.GetString("language")]; 
     ...
     public void toTitleScreen()
    {
        ...
        GameManager.GM.canSkip = false;
        GameManager.GM.menuActive = false;
        GameManager.GM.captainAgreeAnswer = 0;
        Time.timeScale = 1.0f; 
    ...
    public void continueCommand()
    {
         setSelected = true;
            ...
            player = GameObject.FindWithTag("Player");
             ...
             else if (player.GetComponent<PlayerInputSHip>() != null){
             player.GetComponent<PlayerInputSHip>().enabled = true;
             }}
            Time.timeScale = 1.0f;  
            if (cursorState)
            {
    ...
    void Update()
    {
        ...
        if ((Input.GetKeyDown(GameManager.GM.escape) || Input.GetKeyDown(GameManager.GM.escapeJC)) && !menuPanel.gameObject.activeSelf 
        && !PanelVisibility.panel.visible && !AnswerPanelVisibility.panel.visible && !GameManager.GM.alternativeInput && !GameManager.GM.canSkip && !GameManager.GM.textRunning && !GameManager.GM.blackTextScreen){
             if (player != null){
             ...
             player.GetComponent<PlayerInputSHip>().enabled = false;
             }}
            ...
            canChangeVolume = true;
             ...
             player = GameObject.FindWithTag("Player");
            // Debug.Log(menuPanel);
             GameManager.GM.menuActive = true;
             if (setSelected){
                 EventSystem.current.SetSelectedGameObject(ButtonJump);
                 setSelected =false;
             }
        ...
        else if ((Input.GetKeyDown(GameManager.GM.escape) || Input.GetKeyDown(GameManager.GM.escapeJC)) && menuPanel.gameObject.activeSelf && !cantClickGui){
             setSelected = true;
             menuPanel.gameObject.SetActive(false);
            player = GameObject.FindWithTag("Player");
                          GameManager.GM.menuActive = false;
        ...
        if (AnswerPanelVisibility.panel.visible &&
               ( EventSystem.current.currentSelectedGameObject != AnswerPanelVisibility.panel.transform.GetChild(0).gameObject&&
                 EventSystem.current.currentSelectedGameObject !=  AnswerPanelVisibility.panel.transform.GetChild(1).gameObject &&
                 EventSystem.current.currentSelectedGameObject !=  AnswerPanelVisibility.panel.transform.GetChild(2).gameObject
                )       
            )
        {
            Cursor.lockState = CursorLockMode.Locked;
            Cursor.visible = false;
            EventSystem.current.SetSelectedGameObject(AnswerPanelVisibility.panel.transform.GetChild(0).gameObject);
            
        }
        
       if (AnswerPanelVisibility.panel.visible && !AnswerPanelVisibility.panel.transform.GetChild(2).gameObject.activeSelf
       && EventSystem.current.currentSelectedGameObject ==  AnswerPanelVisibility.panel.transform.GetChild(2).gameObject)
            {
              EventSystem.current.SetSelectedGameObject(AnswerPanelVisibility.panel.transform.GetChild(0).gameObject);
            }
       if (AnswerPanelVisibility.panel.visible && !AnswerPanelVisibility.panel.transform.GetChild(1).gameObject.activeSelf
       && EventSystem.current.currentSelectedGameObject ==  AnswerPanelVisibility.panel.transform.GetChild(1).gameObject)
            {
              EventSystem.current.SetSelectedGameObject(AnswerPanelVisibility.panel.transform.GetChild(0).gameObject);
            }
        ...
        //MAIN MENU PANEL
        if (menuPanel.gameObject.activeSelf && MainPanel.activeSelf && EventSystem.current.currentSelectedGameObject != MainPanel.transform.GetChild(0).gameObject &&
             EventSystem.current.currentSelectedGameObject != MainPanel.transform.GetChild(1).gameObject &&
             EventSystem.current.currentSelectedGameObject != MainPanel.transform.GetChild(2).gameObject)
        {
            EventSystem.current.SetSelectedGameObject(MainPanel.transform.GetChild(0).gameObject);
        }
        ...
        if(menuPanel.gameObject.activeSelf && 
            (
             EventSystem.current.currentSelectedGameObject != ButtonJump &&
             EventSystem.current.currentSelectedGameObject != ButtonAttack &&
             EventSystem.current.currentSelectedGameObject != ButtonLeft &&
             EventSystem.current.currentSelectedGameObject != ButtonRight && 
             EventSystem.current.currentSelectedGameObject != ButtonDown
             )
           )
             {EventSystem.current.SetSelectedGameObject(ButtonJump);}
             */
            
        if ((((AnswerPanelVisibility.panel.visible || menuPanel.gameObject.activeSelf) && 
    ...
    void OnGUI()
    {
      ...
      foreach(KeyCode kcode in  System.Enum.GetValues(typeof(KeyCode)))
      {
          ...
          */
       if ((Input.anyKey || keyEvent.functionKey) && waitingForKey && Controller.joystickController.joystick != 1 && !keyEvent.isMouse)
       {
          ...
          {
              Debug.Log("KeyCode down: " +keyEvent.keyCode.ToString());
    ...
    public IEnumerator AssignKey(string keyName){
       ...
       if (PCPanel.activeSelf && newKey.ToString() != "None" && !newKey.ToString().Contains("Joystick") )
        ...
        btnC.color = Color.white;
         WorkWithText.textWork.actionButtonAssign();
