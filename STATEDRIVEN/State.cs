...
public class State
{
    ...
    public int CanSeePlayer()
    {
        ...
        {
            return 1; // NPC CAN see the player.
        }
        else if (direction.magnitude < spineDist && angle > spineAngel)
        {
            return 2;
        }
        return 0; // NPC CANNOT see the player.
    }
...
public class Idle : State
{
    ...
    public override void Update()
    {
        ...
        {
            nextState = new BOXRUN(npc, agent, anim, player);
            stage = EVENT.EXIT; // The next time 'Process' runs, the EXIT stage will run instead, which will then return the nextState.
        }
...
public class BOXRUN : State
{
    
    float cubeDist = 1f;
    public BOXRUN(GameObject _npc, NavMeshAgent _agent, Animator _anim, Transform _player)
                : base(_npc, _agent, _anim, _player)
    {
        name = STATE.PURSUE; // State set to match what NPC is doing.
        agent.speed = 5; // Speed set to make sure NPC appears to be running.
    ...
    public override void Update()
    {
        ...
        box = GameObject.FindWithTag("Safe");
        agent.SetDestination(box.transform.position); // Set goal for NPC to reach but navmesh processing might not have taken place, so...
